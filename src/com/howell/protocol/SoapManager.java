...
public class SoapManager implements Serializable ,IConst{
	...
	public SoapObject initEnvelopAndTransport(SoapObject rpc , String sSoapAction) {
		...
		try {
			...
			Log.e("123", "SocketTimeoutException");
			e.printStackTrace();
		} catch (Exception e) {
			Log.e("123", "Exception");
			e.printStackTrace();
		...
		Log.i("123","bodyin="+envelope.bodyIn.toString());	
	...
	//    运动侦测参数设置
	public String setVMDParam(VMDParamRes res) {
		//TODO
		...
		Object result = "";
		try{
			 result = object.getProperty("result");
			//    	 	if(result.toString().equals("SessionExpired")){
	...
	public GetVideoBasicRes GetVideoBasicRes(GetVideoBasicReq req){
		GetVideoBasicRes res = new GetVideoBasicRes();
		SoapObject rpc = new SoapObject(sNameSpace, "getVideoBasicReq");
		rpc.addProperty("Account", req.getAccount());
		rpc.addProperty("LoginSession", req.getLoginSession());
		rpc.addProperty("DevID", req.getDevID());
		rpc.addProperty("ChannelNo", req.getChannelNO());

		SoapObject object = initEnvelopAndTransport(rpc,"http://www.haoweis.com/HomeServices/MCU/getVideoBasic");
		try {
			Object result = object.getProperty("result");
			if(result.toString().equals("SessionExpired")){
				mLoginResponse = getUserLoginRes(mLoginRequest);
				req.setLoginSession(mLoginResponse.getLoginSession());
				return GetVideoBasicRes(req);
			}
			res.setResult(result.toString());
			Object dnMode = object.getProperty("DNMode");
			if (dnMode.toString().equalsIgnoreCase("Auto")) {
				SoapObject dnSensitivity = (SoapObject) object.getProperty("DNSensitivity");
				Object dnMin = dnSensitivity.getProperty("Minimum");
				res.setDnSensitivityMin(Integer.valueOf(dnMin.toString()));
				Object dnMax = dnSensitivity.getProperty("Maximum");
				res.setDnSensitivityMax(Integer.valueOf(dnMax.toString()));
				Object dnVal = dnSensitivity.getProperty("Value");
				res.setDnSensitivityVal(Integer.valueOf(dnVal.toString()));
			}
			res.setDnMode(dnMode.toString());

			Object agcMode = object.getProperty("AGCMode");
			if (agcMode.toString().equalsIgnoreCase("Manual")) {
				SoapObject agc = (SoapObject) object.getProperty("AGC");
				Object agcMin = agc.getProperty("Minimum");
				res.setAgcMin(Integer.valueOf(agcMin.toString()));
				Object agcMax = agc.getProperty("Maximum");
				res.setAgcMax(Integer.valueOf(agcMax.toString()));
				Object agcVal = agc.getProperty("Value");
				res.setAgcVal(Integer.valueOf(agcVal.toString()));
			}
			res.setAgcMode(agcMode.toString());

			Object eShutterMode = object.getProperty("eShutterMode");
			if (eShutterMode.toString().equalsIgnoreCase("Manual")) {
				SoapObject eShutter = (SoapObject) object.getProperty("eShutter");
				Object val = eShutter.getProperty("Value");
				res.seteShutterVal(val.toString());
				Object options = eShutter.getProperty("Options");
				res.seteShutterOptions(options.toString());
			}
			res.seteShutterMode(eShutterMode.toString());

			Object infraredMode = object.getProperty("InfraredMode");
			res.setInfraredMode(infraredMode.toString());
		} catch (Exception e) {
			// TODO: handle exception
			Log.e("123", "GetVideoBasic error~~");
			e.printStackTrace();
		}
		return res;
	}
	...
	public SetVideoBasicRes SetVideoBasicRes(SetVideoBasicReq req){
		SetVideoBasicRes res = new SetVideoBasicRes();
		SoapObject rpc = new SoapObject(sNameSpace, "setVideoBasicReq");
		rpc.addProperty("Account", req.getAccount());
		rpc.addProperty("LoginSession", req.getLoginSession());
		rpc.addProperty("DevID", req.getDevID());
		rpc.addProperty("ChannelNo", req.getChannelNO());
		if (req.getDnMode()!=null) {
			rpc.addProperty("DNMode", req.getDnMode());
			if (req.getDnMode().equalsIgnoreCase("Auto")) {
				rpc.addProperty("DNSensitivity", req.getDnSensitivityVal());
			}
		}
		if (req.getAgcMode()!=null) {
			rpc.addProperty("AGCMode", req.getAgcMode());
			if (req.getAgcMode().equalsIgnoreCase("Manual")) {
				rpc.addProperty("AGC", req.getAgcVal());
			}
		}
		if (req.geteShutterMode()!=null) {
			rpc.addProperty("eShutterMode", req.geteShutterMode());
			if (req.geteShutterMode().equalsIgnoreCase("Manual")) {
				rpc.addProperty("eShutter", req.geteShutter());
			}
		}
		if (req.getInfraredMode()!=null) {
			rpc.addProperty("InfraredMode", req.getInfraredMode());
		}
		
		SoapObject object = initEnvelopAndTransport(rpc,"http://www.haoweis.com/HomeServices/MCU/setVideoBasic");
		try{
			Object result = object.getProperty("result");
			if(result.toString().equals("SessionExpired")){
				mLoginResponse = getUserLoginRes(mLoginRequest);
				req.setLoginSession(mLoginResponse.getLoginSession());
				return SetVideoBasicRes(req);
			}
			res.setResult(result.toString());
